...
import { TouchableOpacity, StyleSheet, View, StatusBar, Animated, ScrollView, Dimensions, I18nManager } from 'react-native';
...
class Login extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = { isLoading: false, facebookLoading: false, googleLoading: false, login: 100, register: 0, email: 'geeksera.online@gmail.com', password: 'a12345678', top: new Animated.Value(0), center: new Animated.Value(0), bottom: new Animated.Value(0) }
    ...
    _configureGoogleSignIn() {
        GoogleSignin.configure({
            webClientId: '133570530718-0i99i74i307rngog76khao2ln9c8de5q.apps.googleusercontent.com', // client ID of type WEB for your server (needed to verify user ID and offline access)
            ...
            iosClientId: '133570530718-0i99i74i307rngog76khao2ln9c8de5q.apps.googleusercontent.com', // [iOS] optional, if you want to specify the client ID of type iOS (otherwise, it is taken from GoogleService-Info.plist)
        });
    }
    ...
    async facebookSignIn() {
        this.setState({ facebookLoading: true })
        var results = await LoginManager.logInWithPermissions(['public_profile', 'email']).then(
            function (result) {
                return result;
            },
            function (error) {
                ...
                const profileRequest = new GraphRequest(
                    ...
                    (error, result) => {
                        ...
                        if (error) {
                            ...
                            if (result.id) {
                                postData.profile = 'http://graph.facebook.com/' + result.id + '/picture?type=large&height=320&width=420';
                            }
    ...
    render() {
        ...
        return (
            ...
            <View style={styles.container_scrolling}>
                ...
                <Header search={false} back={true} navigation={this.props.navigation} logo={false}
                    backclick={() => { this.props.navigation.pop() }} />
                    ...
                    <View style={{ width: '100%', height: '100%' }}>
                        ...
                        <Animated.View style={{ opacity: top, flex: this.state.login }} >
                            <View style={[styles.container, { flex: this.state.login }]}>
                                <ScrollView showsVerticalScrollIndicator={false} style={{ width: '100%', height: '100%', flexGrow: 1, flex: 1 }}
                                    contentContainerStyle={{ justifyContent: 'center', alignContent: 'center', alignItems: 'center' }}>
                                    <CachedImage source={require('app/assets/assets/logwithname.png')} resizeMode="contain" style={{ height: moderateScale(110), width: moderateScale(150), marginBottom: moderateScale(0), }} />
                                    <Text bold style={{ marginBottom: moderateScale(15), fontSize: moderateScale(25), color: theme.colors.secondary, }}>{translate('Login')}</Text>
                                    <View style={{ width: '100%', justifyContent: 'center', alignContent: 'center', alignItems: 'center', borderWidth: moderateScale(1), borderColor: theme.colors.primary, borderRadius: moderateScale(10), paddingVertical: moderateScale(20) }}>
                                        <View style={{
                                            flex: 1,
                                            paddingStart: '5%',
                                            paddingEnd: '5%',
                                            ...
                                            <Animated.View style={{
                                                ...
                                                <Input
                                                    ...
                                                    inputContainerStyle={{
                                                        ...
                                                        ...I18nManager.isRTL ? { paddingRight: 10 } : {}, borderBottomWidth: 0, backgroundColor: theme.colors.gray06, borderRadius: moderateScale(10), marginTop: moderateScale(20)
                                                    }}
                                                ...
                                                <Input
                                                    ...
                                                    inputContainerStyle={{
                                                        width: '100%', width: '100%', height: 45,
                                                        ...I18nManager.isRTL ? { paddingRight: 10 } : {}, borderBottomWidth: 0, backgroundColor: theme.colors.gray06, marginTop: moderateScale(10), borderRadius: moderateScale(10)
                                                    }}
                                                    inputStyle={{ textAlignVertical: 'center', height: '100%', ...I18nManager.isRTL ? { textAlign: 'right' } : {} }}
                                                    ref={(input) => { this.inputRefs.password = input; }}
                                                    returnKeyType="next"
                                                ...
                                                <Button mode="contained"
                                                    isDisabled={this.state.facebookLoading || this.state.googleLoading}
                                                    ...
                                                    isLoading={this.state.isLoading} style={{
                                                        ...
                                                        backgroundColor: theme.colors.secondary, marginTop: moderateScale(20)
                                                    }} textStyle={{
                                                ...
                                                <View style={[styles.forgotPassword, { alignItems: 'flex-start', alignContent: 'flex-start', ...I18nManager.isRTL ? { alignSelf: 'flex-end' } : { alignSelf: 'flex-start' } }]}>
                                                    <TouchableOpacity style={{ width: '100%', }} onPress={() => { this.props.navigation.navigate('ForgetPassword') }} >
                                                        <Text style={[styles.label, { ...I18nManager.isRTL ? { textAlign: 'right', width: '100%' } : { textAlign: 'left', width: '100%' }, color: theme.colors.secondary, width: '100%' }]}>{translate('Forget Password')}</Text>
                                                    </TouchableOpacity>
                                                </View>
                                            </Animated.View>
                                    ...
                                    <View>
                                        <View resizeMode="contain" source={require('app/assets/backreg.png')} style={{
                                            width: '100%', marginTop: moderateScale(20)
                                        }}>
                                            <TouchableOpacity onPress={() => { this.props.navigation.navigate('Register') }}  >
                                                <Text bold style={{ color: theme.colors.primary, fontSize: moderateScale(20), textAlign: 'center', }}>{translate('Register Now')}</Text>
                                            </TouchableOpacity>
                                        </View>
                                    </View>
...
const styles = StyleSheet.create({
    ...
    separatorLine: {
        ...
        borderWidth: 0.5, marginHorizontal: 10
    },
    separatorOr: {
        color: theme.colors.primary,
        marginHorizontal: 20,
    ...
    ImageIconStyle: {
        height: 25,
        width: 25,
        resizeMode: 'stretch', alignSelf: 'center', marginEnd: moderateScale(15), marginStart: moderateScale(5)
    },
    TextStyle: {
